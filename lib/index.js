class __TreeStore {
    ...
    keyValue () {
	    ...
	    ;(function getKeyValue (data){
			data.forEach(function(item){
				rel_keyValue[item.id] = item.value
				if(Array.isArray(item.child)){
    ...
    childAllIds () {
		...
		let rel_childAllIds = JSON.parse(JSON.stringify((this.childIds())))//深复制 子元素关系表
...
//获取当前id对应value
__TreeStore.prototype.getValue = function (id) {
	...
	if(Array.isArray(id)){
		...
		for(let i in id){
			...
			result.push(this.keyValue()[id[i]])
		}
		return result
	}
	//检测是否数字类型并转化
	id = this.idTestString(id)
    return this.keyValue()[id]
}
//获取当前id所在item所有数据
...
__TreeStore.prototype.getParentAllIds = function (id) {
	...
	if(Array.isArray(id)){
		...
		for(let i in id){
			id[i] = this.idTestString(id[i])
			result.push(this.parentAllIds()[id[i]])
		}
		return result
	}
	//检测是否数字类型并转化
...
__TreeStore.prototype.renderSelect = function (settings) {
	var self = this
	//检测checkIds必须为数组
	if(!Array.isArray(settings.checkedIds)){
		console.warn(settings.checkedIds+'must not be Array')
		return settings.checkedIds
	}
	//深复制checkIds
	var ids = JSON.parse(JSON.stringify(settings.checkedIds))
	//传index时,清空后续id && 补全前置空id
	if(typeof settings.index != 'undefined'){
		for(var i in ids){
			if(i > settings.index){
				ids[i] = ''
			}else if(i < settings.index && ids[i] == ''){
				ids[i] = self.getChildIds(ids[i - 1])[0]
			}
		}
	}
	//遍历生成renderList
	...
	ids.forEach(function(item,index){
		...
		if(/\S/.test(item) || index == 0 ){
			ids_list.push(self.getSiblingIds(item))
		}else{
			if(/\S/.test(ids[index - 1])){
				let parent_id = ids[index - 1]
				ids_list = ids_list.concat(self.getChildLeftBranchIds(parent_id))
			}
		}
	})
	//数据层数和级数不匹配时
	var length_diff = ids.length - ids_list.length
	if(length_diff > 0){
		for(var i = 0 ; i < ids.length ; i++){
			if(i >= ids_list.length){
				ids_list.push([])
			}
		}
	}else if(length_diff < 0){
		ids_list = ids_list.slice(0,ids.length - 1)
	}
...
__TreeStore.prototype.changeChecked = function (settings) {
	...
	if(isChecked){//更新取消选中unchecked_ids并去除checked存在项
		let temp_ids = this.childAllIds()[id]
		temp_ids.push(id)
		let self = this
		//递归遍历所有需要去除的祖父ids
	...
	}else{//添加选中checked_ids并去重checked
		let temp_ids = this.childAllIds()[id]
		temp_ids.push(id)
		temp_ids = temp_ids.concat(this.parentAllIds()[id]).sort()

		let temp_checked_ids = JSON.parse(JSON.stringify(checkedIds))
		temp_checked_ids = temp_checked_ids.concat(temp_ids).sort()

		for(let i in temp_checked_ids){
			if(temp_checked_ids[i] !== temp_checked_ids[ i - 1 ] && i > 0){
				result.checked.push(temp_checked_ids[i - 1])
				if( i  == temp_checked_ids.length - 1){
					result.checked.push(temp_checked_ids[ i ])
				}
			}
		}
...
function TreeStore(data,settings) {
    ...
    //数据key转换
    if(settings !== false){
	    settings.data = JSON.parse(JSON.stringify(data))//深复制数据源
    	settings.data = TreeStore.treeMap(settings.data,settings.child,function(item){
    		if(typeof settings.id != 'undefined'){
    			item.id = item[settings.id]
    			delete item[settings.id]
    		}
    		if(typeof settings.value != 'undefined'){
    			item.value = item[settings.value]
    			delete item[settings.value]
    		}
    		if(typeof settings.child != 'undefined'){
    			item.child = item[settings.child]
    			delete item[settings.child]
    		}
    		return item
    	})
    }else{
    	settings = {
    		data : JSON.parse(JSON.stringify(data))//深复制数据源
    	}
    }
...
TreeStore.treeMap = function (data,childName,fn) {
    let temp_data = JSON.parse(JSON.stringify((data)))//深复制数据源
