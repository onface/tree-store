...
__TreeStore.prototype.renderSelect = function (settings) {
    ...
    if(typeof settings.minLength != 'undefined'){
        let minLength = Number(settings.minLength)
        if(minLength > 10){
            console.warn('minLength too long');
        }
        if(minLength > 0){
            while(ids_list.length < minLength){
                ids_list.push([])
            }
        }else{
            console.warn('minLength must be valid Number');
        }
    }
    //需要填充数据
    if(typeof settings.filObj != 'undefined'){
        settings.filObj = settings.filObj || []
        if((settings.filObj instanceof Object) && !(settings.filObj instanceof Array) ){
            let obj = extend(true,{},settings.filObj)
            ids_list = ids_list.map(function(item){
                item.unshift(obj)
                return item
            })
        }
